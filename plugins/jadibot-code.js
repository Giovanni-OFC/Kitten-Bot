...
if (!fs.existsSync("./KittenJadiBot/" + authFolderB)) {
fs.mkdirSync("./KittenJadiBot/" + authFolderB, { recursive: true });
}
if (args[0]) {
fs.writeFileSync(`GenesisJadiBot/creds.json`, Buffer.from(args[0], 'base64').toString('utf-8'))
}
const { state, saveState, saveCreds } = await useMultiFileAuthState(`./KittenJadiBot/${authFolderB}`);
